...
@icon("res://assets/icons/SceneRandomizer2D.svg")
@tool class_name SceneRandomizer2D extends Node2D
...
var scene_rotation : float :
	get :
		if pool.random_angle : return get_random_angle()
		else : return 0.0
...
func get_random_angle() -> float : 
	return random.randf_range(0.0, 2.0 * PI)
